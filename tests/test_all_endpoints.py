...
async def test_full_product_flow():
    async with httpx.AsyncClient(headers=HEADERS) as client:
        ...
        assert get_resp.json()["name"] == "Test Item2"

        #Update product
        update_resp = await client.put(f"{BASE_URL}/products/{product_id}", json={
            "name": "Updated Test Item2",
            "description": "Updated description",
            "price": 39.99
        })
        print('update res ', update_resp)
        assert update_resp.status_code == 200
        assert update_resp.json()["name"] == "Updated Test Item2"
...
@pytest.mark.asyncio
async def test_throttling_limit():
    async with httpx.AsyncClient(headers=HEADERS) as client:
        for i in range(101):
            response = await client.get(f"{BASE_URL}/products/invalid_id")
            if response.status_code == 429:
                assert "Rate limit exceeded" in response.text
                break
        else:
            pytest.fail("Throttling not enforced after 100 requests")

@pytest.mark.asyncio
async def test_throttling_separate_limits():
    async with httpx.AsyncClient(headers={"X-Client-Region": "EU"}) as client:
        for i in range(1001):
            response = await client.get(f"{BASE_URL}/products/invalid_id")
            if response.status_code == 429:
                assert "Rate limit exceeded" in response.text
            ...
            pytest.fail("Region-based throttling not enforced after 1000 requests")
